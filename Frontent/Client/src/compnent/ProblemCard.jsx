    ...
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm" >
        <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded-full bg-gray-100 px-3 py-1 text-sm">{problem.difficulty}</span>

        </div>
        <p className="mb-4 text-gray-600">{problem.description}</p>
        <div className="mb-4 flex flex-wrap gap-2">
            {problem.expectedConcepts.map((concept)=> (
                <span key={concept} className="rounded-md bg-blue-50 px-2 py-1 text-xs text-blue-700">{concept}</span>
            ))}
        </div>
        <button
