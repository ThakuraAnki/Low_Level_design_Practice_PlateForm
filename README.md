...
Provide a small collection of LLD problems.
Allow learners to start a practice attempt.
Allow learners to submit a text-based LLD solution.
Evaluate the submission using a structured rubric.
Provide explainable feedback instead of only a score.
Store previous attempts and evaluations.
Allow learners to review feedback and try again.
Keep the evaluation layer replaceable so more advanced evaluators can be added later.
Features
LLD Problem Library
